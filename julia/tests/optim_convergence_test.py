...
from julia.core.ops import Add, Sub, Mul, MatMul # Import Ops
...
optimizer_configs = [
    ...
    pytest.param(SGD, {'lr': 0.1, 'momentum': 0.9, 'nesterov': True}, True, id="SGD_nesterov"),
    pytest.param(SGD, {'lr': 0.1, 'weight_decay': 0.01}, True, id="SGD_weight_decay"),
    pytest.param(AdamW, {'lr': 0.01, 'weight_decay': 0.0, 'amsgrad': True}, True, id="AdamW_amsgrad"),
...
@pytest.mark.parametrize("optimizer_class, optimizer_kwargs, expected_to_converge", optimizer_configs)
def test_optimizer_convergence(linear_regression_data, optimizer_class, optimizer_kwargs, expected_to_converge):
    """
    ...
    print(f"\nTesting {optimizer_class.__name__} with {optimizer_kwargs}") # For clarity during verbose runs
    ...
    for epoch in range(n_epochs):
        ...
        y_pred = Add.apply(y_pred, b) # Broadcasting should handle adding (100,1) and (1,)
        ...
        assert np.all(np.isfinite(W.data)), f"W has NaN/Inf after step() in epoch {epoch}"
        assert np.all(np.isfinite(b.data)), f"b has NaN/Inf after step() in epoch {epoch}"


        if (epoch + 1) % 50 == 0:
             print(f"Epoch {epoch+1}, Loss: {loss.data.item():.6f}, W: {W.data.flatten()}, b: {b.data.item():.4f}")
    ...
    assert final_loss < initial_loss or math.isclose(final_loss, initial_loss, rel_tol=1e-5), \
    ...
    if final_b_data.size == 1:
         assert not math.isclose(final_b_data.item(), initial_b_data.item(), abs_tol=1e-8), \
    ...
    assert np.all(np.isfinite(final_W_data)), f"{optimizer_class.__name__}: Parameter W contains NaN or Inf"
    assert np.all(np.isfinite(final_b_data)), f"{optimizer_class.__name__}: Parameter b contains NaN or Inf"
