...
from julia.core.loss import (
    LossFunction, CustomLossTemplate, LossFunctionFactory,
    MultiTaskLoss, mse_loss, mae_loss
)
...
class TestBaseLossFunction(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.y_pred = Tensor(np.random.randn(self.batch_size, self.output_dim), requires_grad=True)
    ...
    def test_custom_loss_training(self):
        ...
        class RobustLoss(CustomLossTemplate):
            ...
            def forward(self, y_pred, y_true):
                ...
                loss_data[linear_mask] = self.delta * (abs_diff.data[linear_mask] - 0.5 * self.delta)
...
class TestMultiTaskLoss(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.task1_pred = Tensor(np.random.randn(self.batch_size, 5), requires_grad=True)
        self.task1_true = Tensor(np.random.randn(self.batch_size, 5))
        
        self.task2_pred = Tensor(np.random.randn(self.batch_size, 3), requires_grad=True)
        self.task2_true = Tensor(np.random.randn(self.batch_size, 3))
        
        self.task3_pred = Tensor(np.random.randn(self.batch_size, 1), requires_grad=True)
    ...
    def test_multi_task_loss_training(self):
        ...
        all_params = (shared_backbone.parameters() + 
...
class TestLossIntegration(unittest.TestCase):
    ...
    def test_loss_function_composition(self):
        ...
        class CompositeLoss(CustomLossTemplate):
            def __init__(self, loss1, loss2, weight1=0.5, weight2=0.5, reduction='mean'):
                super().__init__(reduction)
                self.loss1 = loss1
                self.loss2 = loss2
