...
from julia.core.profiler import (
    profiler,
    profile_scope,
    enable_profiling,
        ...
        with profiler.profile_operation(
            ...
            if isinstance(result, Tensor):
                ...
                stats.flops = profiler._estimate_flops(
                    cls.__name__, stats.input_shapes, stats.output_shapes
                )
            elif isinstance(result, (list, tuple)):
                stats.output_shapes = [
                    getattr(r, "shape", ()) for r in result if hasattr(r, "shape")
                ]
                stats.output_dtypes = [
                    str(getattr(r, "dtype", "unknown"))
                    for r in result
                    if hasattr(r, "data")
                ]
...
class ProfiledLinear(Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        with profiler.profile_operation(
            f"Linear_{self.in_features}x{self.out_features}", "Linear", [x]
        ):
