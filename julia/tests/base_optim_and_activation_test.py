...
def test_optimizers():
    ...
    for name, optimizer_class in optimizers.items():
        ...
        for epoch in range(200):
            ...
            if epoch % 50 == 0:
                print(f"Epoch {epoch}, Loss: {loss:.6f}, W: {W.data.flatten()[0]:.4f}, b: {b.data.flatten()[0]:.4f}")
        
        print(f"Final parameters - W: {W.data.flatten()[0]:.4f}, b: {b.data.flatten()[0]:.4f}")
...
def test_neural_network():
    ...
    for epoch in range(200):
        ...
        # Compute softmax
        exp_scores = Tensor(np.exp(scores.data - np.max(scores.data, axis=1, keepdims=True)))
