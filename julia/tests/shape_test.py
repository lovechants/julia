...
def test_shape_inference_broadcasting():
    ...
    a = graph.add_placeholder(shape=(3, 4), dtype=DataType.FLOAT32, name="a")
    b = graph.add_placeholder(shape=(4,), dtype=DataType.FLOAT32, name="b")  # Will broadcast
    c = graph.add_placeholder(shape=(1, 4), dtype=DataType.FLOAT32, name="c")  # Will broadcast
...
def test_shape_inference_matmul(): #TODO like actually fix this one or just test the operations more 
    ...
    batch_matrix = graph.add_placeholder(shape=(2, 3, 4), dtype=DataType.FLOAT32, name="batch_matrix")
    matrix2 = graph.add_placeholder(shape=(4, 6), dtype=DataType.FLOAT32, name="matrix2")
...
def test_shape_inference_reshape():
    ...
    # Check correctness
    assert reshape1.shape == (32, 28, 28), f"Expected (32, 28, 28), got {reshape1.shape}"
    assert reshape2.shape == (32, 28, 28), f"Expected (32, 28, 28), got {reshape2.shape}"
