...
from julia.core.ops_nn import MaxPool2DFunction, AdaptiveMaxPool2DFunction, AvgPool2DFunction, GlobalAvgPool2DFunction, GlobalMaxPool2DFunction, AdaptiveAvgPool2DFunction
class TestPoolingFunctions(unittest.TestCase):
    ...
    def test_avgpool2d_with_padding(self):
        ...
        output = AvgPool2DFunction.apply(x_tensor, kernel_size, stride, padding, count_include_pad=True)
        ...
        x_tensor.grad = None
        output = AvgPool2DFunction.apply(x_tensor, kernel_size, stride, padding, count_include_pad=False)
        output.backward(Tensor(grad_output))
    ...
    def test_multiple_channels(self):
        ...
        x = np.random.randn(2, 3, 8, 8).astype(np.float32)  # 2 batches, 3 channels, 8x8 spatial
        x_tensor = Tensor(x, requires_grad=True)
