...
class TestConvLayers(unittest.TestCase):
    def test_conv2d_forward(self):
        ...
        conv.weight = Tensor(np.ones((out_channels, in_channels, kernel_size, kernel_size)), requires_grad=True)
        ...
        expected_corner_value = (ones_in_corner_receptive_field * in_channels) + bias_val # 4 * 3 + 1 = 13.0
        ...
        expected_edge_value = (ones_in_edge_receptive_field * in_channels) + bias_val # 6 * 3 + 1 = 19.0
        ...
        self.assertAlmostEqual(actual_corner, expected_corner_value, places=5,
                               msg=f"Corner pixel [0,0,0,0] mismatch.")

        # Check top edge (non-corner)
        actual_top_edge = output_data[0, 0, 0, 1]
        print(f"Actual Top Edge [0,0,0,1]: {actual_top_edge}")
        self.assertAlmostEqual(actual_top_edge, expected_edge_value, places=5,
                               msg=f"Top edge pixel [0,0,0,1] mismatch.")

        # Check left edge (non-corner)
        actual_left_edge = output_data[0, 0, 1, 0]
        print(f"Actual Left Edge [0,0,1,0]: {actual_left_edge}")
        self.assertAlmostEqual(actual_left_edge, expected_edge_value, places=5,
                               msg=f"Left edge pixel [0,0,1,0] mismatch.")

        # Check a central pixel
        actual_center = output_data[0, 0, 1, 1]
        print(f"Actual Center [0,0,1,1]: {actual_center}")
        self.assertAlmostEqual(actual_center, expected_center_value, places=5,
                               msg=f"Center pixel [0,0,1,1] mismatch.")
        ...
        self.assertTrue(np.allclose(center_output_region, expected_center_value),
                        msg=f"Central region values mismatch. Expected all to be {expected_center_value}")
        print(f"Central region check passed (all values approx {expected_center_value}).")

        # 5. Optional: Check all corners and edges explicitly if desired
        # Check all 4 corners of the first image, first channel
        corners = [output_data[0, 0, 0, 0], output_data[0, 0, 0, -1],
                   output_data[0, 0, -1, 0], output_data[0, 0, -1, -1]]
        self.assertTrue(np.allclose(corners, expected_corner_value),
                        msg=f"Not all corner values match expected {expected_corner_value}")
        print(f"All 4 corner values check passed (all values approx {expected_corner_value}).")

        # Check top/bottom edge regions (excluding corners)
        top_edge_region = output_data[:, :, 0, 1:-1]
        bottom_edge_region = output_data[:, :, -1, 1:-1]
        self.assertTrue(np.allclose(top_edge_region, expected_edge_value),
                        msg=f"Top edge region values mismatch. Expected all to be {expected_edge_value}")
        self.assertTrue(np.allclose(bottom_edge_region, expected_edge_value),
        ...
        self.assertTrue(np.allclose(left_edge_region, expected_edge_value),
                        msg=f"Left edge region values mismatch. Expected all to be {expected_edge_value}")
        self.assertTrue(np.allclose(right_edge_region, expected_edge_value),
                        msg=f"Right edge region values mismatch. Expected all to be {expected_edge_value}")
        print(f"All edge regions check passed (all values approx {expected_edge_value}).")
            ...
            for c in range(channels):
                for h in range(expected_height):
                    for w in range(expected_width):
                        ...
                        expected_max = x_data[b, c, h*pool_size+pool_size-1, w*pool_size+pool_size-1]
                    ...
                    for w in range(expected_width):
                        ...
                        for ph in range(pool_size):
                            for pw in range(pool_size):
                                window_sum += x_data[b, c, h*pool_size+ph, w*pool_size+pw]
                        expected_avg = window_sum / (pool_size * pool_size)
                        self.assertAlmostEqual(output.data[b, c, h, w], expected_avg, places=5)
    ...
    def test_depthwise_separable_conv2d(self):
        ...
        conv = DepthwiseSeparableConv2D(in_channels, out_channels, kernel_size, padding=1)
