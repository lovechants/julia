...
def test_conv2d_pytorch_comparison(in_c, out_c, kernel, stride, padding, bias, batch, h, w):
    """
    ...
    torch_output_np = torch_output.detach().numpy() # Get NumPy array from PyTorch tensor
    ...
    assert np.allclose(julia_output_np, torch_output_np, rtol=1e-5, atol=1e-6), \
        f"Value mismatch between Julia and PyTorch outputs."
