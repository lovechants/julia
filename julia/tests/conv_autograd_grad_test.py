...
from julia.core.nn.conv import (
    ...
    DepthwiseSeparableConv2D, DilatedConv2D
)




def compute_numerical_gradient(tensor_to_check: Tensor,
    ...
    """
    numerical_grad = np.zeros_like(tensor_to_check.data, dtype=np.float64) # Use float64 for precision
...
class TestConvGradientCheck(unittest.TestCase):
    ...
    def _check_gradients(self, layer, x, layer_name="Layer"):
        ...
        def compute_loss():
            ...
            return loss_tensor.data.item() if loss_tensor.data.ndim == 0 else loss_tensor.data

        # 1. Compute Analytical Gradients
        # Zero grads first
        x.zero_grad()
        for p in params_to_check: p.zero_grad()
        ...
        for i, p in enumerate(params_to_check):
            print(f"  Computing numerical gradient for parameter {i} (shape {p.shape})...")
        ...
        self.assertTrue(
            np.allclose(analytic_grads['x'], numerical_grads['x'], rtol=1e-4, atol=1e-5),
            f"{layer_name} Input 'x' gradient mismatch:\nAnalytic:\n{analytic_grads['x']}\nNumerical:\n{numerical_grads['x']}"
        )
        # Compare parameter gradients
        for i, p in enumerate(params_to_check):
            param_key = f'param_{i}'
            self.assertIsNotNone(analytic_grads[param_key], f"Analytical grad for param {i} is None")
            self.assertTrue(
                np.allclose(analytic_grads[param_key], numerical_grads[param_key], rtol=1e-4, atol=1e-5),
