...
class TestConvAutograd(unittest.TestCase):
    def test_conv1d_forward_backward(self):
        ...
        # Create input tensor
        x = Tensor(
            np.random.randn(batch_size, in_channels, seq_len), requires_grad=True
        )
    ...
    def test_conv2d_forward_backward(self):
        ...
        # Create input tensor
        x = Tensor(
            np.random.randn(batch_size, in_channels, height, width), requires_grad=True
        )
        ...
        self.assertEqual(
            y.shape, (batch_size, out_channels, expected_h_out, expected_w_out)
        )
    ...
    def test_conv3d_forward_backward(self):
        ...
        # Create input tensor
        x = Tensor(
            np.random.randn(batch_size, in_channels, depth, height, width),
            requires_grad=True,
        )
        ...
        self.assertEqual(
            y.shape,
            (batch_size, out_channels, expected_d_out, expected_h_out, expected_w_out),
        )
    ...
    def test_transposed_conv2d_forward_backward(self):
        ...
        # Ensure output has the correct shape
        expected_h_out = (
            (height - 1) * stride - 2 * padding + kernel_size + output_padding
        )
        expected_w_out = (
            (width - 1) * stride - 2 * padding + kernel_size + output_padding
        )
        self.assertEqual(
            y.shape, (batch_size, out_channels, expected_h_out, expected_w_out)
        )
    ...
    def test_depthwise_separable_conv2d_forward_backward(self):
        ...
        conv = DepthwiseSeparableConv2D(
            in_channels, out_channels, kernel_size, stride, padding, depth_multiplier
        )
