...
class DivMod(Function):
    ...
    def backward(ctx, grad_quotient, grad_remainder):
        ...
        if grad_quotient is not None:
            ...
            grad_divisor_from_quotient = (
                -grad_quotient.data * dividend.data / (divisor.data**2)
            )
        ...
        if grad_remainder is not None:
            ...
            grad_divisor_from_remainder = -grad_remainder.data * np.floor_divide(
                dividend.data, divisor.data
            )
...
def test_multi_output_autograd():
    ...
    # Test Split function
    x = Tensor(
        np.array([[1, 2, 3, 4], [5, 6, 7, 8]], dtype=np.float32), requires_grad=True
    )
