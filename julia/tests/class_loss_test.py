...
class TestClassificationLosses(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.y_binary_true = Tensor(np.random.randint(0, 2, (self.batch_size, 1)).astype(float))
    ...
    def test_cross_entropy_loss_forward(self):
        ...
        expected_loss = -np.mean(np.log(softmax[np.arange(self.batch_size), self.y_true.data] + 1e-8))
    ...
    def test_focal_loss_gamma_effect(self):
        ...
        for gamma in gammas:
            loss = focal_loss(self.y_binary_pred, self.y_binary_true, alpha=1.0, gamma=gamma)
    ...
    def test_focal_loss_class_imbalance(self):
        ...
        focal_loss_val = focal_loss(self.y_binary_pred, y_imbalanced, alpha=1.0, gamma=2.0)
    ...
    def test_accuracy_improvement(self):
        ...
        self.assertGreater(final_acc, initial_acc,
                         "Accuracy did not improve during training")
        self.assertGreater(final_acc, 0.7,  # Should achieve reasonable accuracy
    ...
    def test_loss_reduction_modes(self):
        ...
        loss_none = BinaryCrossEntropyLoss.apply(self.y_binary_pred, self.y_binary_true, 'none')
        loss_mean = BinaryCrossEntropyLoss.apply(self.y_binary_pred, self.y_binary_true, 'mean')
        loss_sum = BinaryCrossEntropyLoss.apply(self.y_binary_pred, self.y_binary_true, 'sum')
