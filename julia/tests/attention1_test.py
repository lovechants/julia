...
def test_self_attention():
    ...
    print(f"  Parameter gradient norms: min={min(param_grad_norms):.6f}, max={max(param_grad_norms):.6f}")
...
def test_training_loop():
    ...
    # Create model
    attention = MultiHeadAttention(d_model, num_heads, dropout=0.0)  # No dropout for deterministic test
    ...
    for epoch in range(num_epochs):
        ...
        key = Tensor(np.random.randn(batch_size, seq_len, d_model), requires_grad=True)
        value = Tensor(np.random.randn(batch_size, seq_len, d_model), requires_grad=True)
        target = Tensor(np.random.randn(batch_size, seq_len, d_model), requires_grad=False)
    ...
    print(f"Parameter norms after training: min={min(param_norms):.6f}, max={max(param_norms):.6f}")
