...
class Metric(ABC):
    ...
    @abstractmethod
    def update(
        self, predictions: Union[Tensor, np.ndarray], targets: Union[Tensor, np.ndarray]
    ):
...
class Precision(Metric):
    ...
    def update(
        ...
        if self.num_classes is None:
            ...
            for i in range(self.num_classes):
                self.true_positives[i] += np.sum(
                    (pred_labels == i) & (target_data == i)
                )
    ...
    def compute(self) -> float:
        if self.num_classes is None:
            return (
                self.true_positives / self.predicted_positives
            ...
            if self.average == "macro":
                ...
                return (
                    np.sum(self.true_positives) / np.sum(self.predicted_positives)
                    if np.sum(self.predicted_positives) > 0
                    else 0.0
                )
            elif self.average == "weighted":
                weights = (
                    self.predicted_positives / np.sum(self.predicted_positives)
                    if np.sum(self.predicted_positives) > 0
                    else np.zeros_like(self.predicted_positives)
                )
...
class Recall(Metric):
    ...
    def compute(self) -> float:
        if self.num_classes is None:
            return (
                self.true_positives / self.actual_positives
            ...
            if self.average == "macro":
                ...
                return (
                    np.sum(self.true_positives) / np.sum(self.actual_positives)
                    if np.sum(self.actual_positives) > 0
                    else 0.0
                )
            elif self.average == "weighted":
                weights = (
                    self.actual_positives / np.sum(self.actual_positives)
                    if np.sum(self.actual_positives) > 0
                    else np.zeros_like(self.actual_positives)
                )
    ...
    def compute(self) -> float:
        ...
        if isinstance(p, np.ndarray):
            ...
            if self.average == "macro":
                ...
                weights = (
                    self.recall.actual_positives / np.sum(self.recall.actual_positives)
                    if np.sum(self.recall.actual_positives) > 0
                    else np.zeros_like(self.recall.actual_positives)
                )
...
class R2Score(Metric):
    ...
    def compute(self) -> float:
        ...
        return (
            1.0 - (self.sum_squared_residuals / self.total) / (target_mean**2)
            if target_mean != 0
            else 0.0
        )
...
class ConfusionMatrix:
    ...
    def update(
        ...
        for true_label, pred_label in zip(target_data, pred_labels):
            if (
                0 <= true_label < self.num_classes
                and 0 <= pred_label < self.num_classes
            ):
...
def accuracy_score(
    predictions: Union[Tensor, np.ndarray], targets: Union[Tensor, np.ndarray]
) -> float:
...
def classification_report(
    ...
    report = {
        ...
        "weighted_precision": precision_score(
            predictions, targets, "weighted", num_classes
        ),
