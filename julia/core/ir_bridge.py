    ...
    def _process_tensor(self, tensor: Tensor, graph: IRGraph) -> IRNode:
        ...
        if tensor._backward_node is None:
            ...
            if tensor.requires_grad:
                ...
                node = graph.add_constant(
                    tensor.data.copy(), name=f"const_{tensor.id[:8]}"
                )
        ...
        for input_tensor in backward_node.inputs:
            if isinstance(input_tensor, Tensor):
                ...
                node = graph.add_constant(
                    np.array(input_tensor), name=f"const_{hash(str(input_tensor))}"
                )
        ...
        node = IRNode(
            op_type=op_type, inputs=input_nodes, name=f"{op_type}_{tensor.id[:8]}"
        )
...
class IRToTensor:
    ...
    def _process_node(
        ...
        Tensor
    ):  # TODO move this to registry implementation instead of hardcoding the operations (especially with new ops up)
        ...
        if node.op_type == "Constant":
            ...
            value = node.attributes.get(
                "value", np.zeros(node.shape, dtype=node.dtype.to_numpy())
            )
            ...
            tensor = Tensor(
                np.zeros(node.shape, dtype=node.dtype.to_numpy()), requires_grad=False
            )

        else:
            # For operations, first process input tensors
            input_tensors = [
                self._process_node(input_node) for input_node in node.inputs
            ]
...
def execute_graph(
    graph: IRGraph, input_dict: Dict[str, Tensor]
