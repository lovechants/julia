...
class MemoryPool:
    ...
    def __init__(self):
        ...
        self.allocated_blocks: Dict[int, Tuple[ctypes.c_void_p, int]] = {}  # ptr_value -> (ptr, size)
    ...
    def allocate(self, byte_size: int) -> Optional[ctypes.c_void_p]:
        ...
        with self._pool_lock:
            try:
                ...
                self.peak_memory = max(self.peak_memory, self.total_allocated - self.total_freed)
        ...
        with self._pool_lock:
            try:
                ...
                if stored_size != byte_size:
                    print(f"Warning: Size mismatch on deallocation: {stored_size} vs {byte_size}")
    ...
    def get_stats(self):
        with self._pool_lock:
            ...
            return {
                ...
                'pool_sizes': {size: len(blocks) for size, blocks in self.free_blocks.items()}
            }
