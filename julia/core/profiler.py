...
class ModelProfiler:
    ...
    def _init_aliyah(self):
        try:
            ...
            print(
                "Aliyah not abailable for TUI visualization, install with pip install aliyah or disable visualization tracking"
            )
    ...
    def _estimate_flops(
        self, op_type: str, input_shapes: List[tuple], output_shapes: List[tuple]
    ) -> Optional[int]:
    ...
    def profile_operation(
        ...
        if inputs:
            ...
            stats.input_dtypes = [
                str(getattr(inp, "dtype", "unknown")) for inp in inputs
            ]
    ...
    def _update_aggregated_stats(self, stats: OperationStats):
        ...
        if key not in self.aggregated_stats:
            self.aggregated_stats[key] = AggregatedStats(
                op_name=stats.op_name, op_type=stats.op_type
            )
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                with self.profile_operation(
                    func_name, op_type or func.__name__, inputs
                ):
                    return func(*args, **kwargs)
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            top_ops_by_time = sorted(
                self.aggregated_stats.values(), key=lambda x: x.total_time, reverse=True
            )[:10]

            # Top operations by call count
            top_ops_by_count = sorted(
                self.aggregated_stats.values(), key=lambda x: x.call_count, reverse=True
            )[:10]
    ...
    def print_summary(self, top_n: int = 10):
        ...
        print("-" * 80)
        print(
            f"{'Operation':<30} {'Type':<15} {'Time(s)':<10} {'Count':<8} {'Avg(ms)':<10} {'%':<6}"
        )
        print("-" * 80)

        for op in summary["top_operations_by_time"][:top_n]:
            print(
                f"{op['name'][:29]:<30} {op['type'][:14]:<15} "
        ...
        print("-" * 80)
        print(
            f"{'Operation':<30} {'Type':<15} {'Count':<8} {'Time(s)':<10} {'Avg(ms)':<10}"
        )
        print("-" * 80)
        ...
        with self.lock:
            for key, stats in self.aggregated_stats.items():
                if stats.avg_memory > 10 * 1024 * 1024:  # > 10MB average increase
                    leaks.append(
                        f"Operation {key} averages {stats.avg_memory/1024/1024:.2f} MB increase"
                    )
