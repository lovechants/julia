    ...
    def _format_message(self) -> str:
        ...
        if self.context:
            ...
            if self.context.memory_usage:
                lines.append(
                    f"  Memory Usage: {self.context.memory_usage / 1024 / 1024:.2f} MB"
                )
...
class TensorError(JuliaError):
    ...
    def __init__(
        self, message: str, tensors: List = None, operation: str = None, **kwargs
    ):
    ...
    def _generate_shape_suggestion(self, expected, actual) -> str:
        ...
        if len(expected) != len(actual):
            if len(actual) < len(expected):
                ...
                suggestions.append(
                    "Consider using squeeze() or view() to reduce dimensions"
                )
        ...
        if len(expected) == len(actual):
            for i, (e, a) in enumerate(zip(expected, actual)):
                if e != a and e == 1:
                    suggestions.append(
                        f"Try expanding dimension {i} of expected tensor"
                    )
                elif e != a and a == 1:
        ...
        if not suggestions:
            try:
                ...
                suggestions.append(
                    "Shapes are broadcastable - this might be an internal error"
                )
...
class BackendError(JuliaError):
    ...
    def __init__(self, message: str, backend: str = None, **kwargs):
        ...
        kwargs.setdefault(
            "error_code", f"{backend.upper()}_ERROR" if backend else "BACKEND_ERROR"
        )
        super().__init__(message, **kwargs)
...
class LayerError(JuliaError):
    ...
    def __init__(
        self, message: str, layer_name: str = None, layer_type: str = None, **kwargs
    ):
...
class ONNXError(JuliaError):
    ...
    def __init__(self, message: str, **kwargs):
        kwargs.setdefault(
            "suggestion", "Check ONNX model format and ensure onnx package is installed"
        )
...
def validate_backend_available(backend_name, available):
    ...
    if not available:
        ...
        suggestion = suggestions.get(
            backend_name, f"Install {backend_name} dependencies"
        )
