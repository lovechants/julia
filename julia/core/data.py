...
class CSVDataset(Dataset):
    ...
    def __init__(
        ...
        self.data = np.loadtxt(
            file_path, delimiter=delimiter, skiprows=1 if header else 0, dtype=dtype
        )
        ...
        if feature_columns is not None:
            if isinstance(feature_columns[0], str):
                self.feature_indices = [
                    self.column_names.index(col) for col in feature_columns
                ]
            else:
            ...
            if self.target_idx is not None:
                self.feature_indices = [
                    i for i in range(self.data.shape[1]) if i != self.target_idx
                ]
                self.features = self.data[:, self.feature_indices]
            else:
...
class ToTensor(Transform):
    ...
    def __call__(self, sample: Any) -> Any:
        if isinstance(sample, (list, tuple)):
            return tuple(
                Tensor(np.array(item, dtype=self.dtype))
        ...
        if isinstance(sample, (list, tuple)):
            ...
            for item in sample:
                if isinstance(item, (Tensor, np.ndarray)):
                    ...
                    result.append(
                        Tensor(normalized) if isinstance(item, Tensor) else normalized
                    )
                else:
                    result.append(item)
            return tuple(result)
                ...
                for item in sample:
                    if isinstance(item, (Tensor, np.ndarray)):
                        ...
                        if len(data.shape) >= 2:
                            ...
                            result.append(
                                Tensor(flipped) if isinstance(item, Tensor) else flipped
                            )
                        else:
                            result.append(item)
                    else:
...
class RandomSampler(Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        if self.replacement:
            return iter(
                np.random.choice(len(self.dataset), self.num_samples, replace=True)
            )
        else:
...
def collate_fn_default(batch: List[Any]) -> Any:
    ...
    if isinstance(batch[0], (list, tuple)):
        return tuple(
            collate_fn_default([sample[i] for sample in batch])
...
class DataLoaderProfiler:
    ...
    def profile(self, num_epochs: int = 1) -> Dict[str, float]:
        ...
        return {
            ...
            "samples_per_second": (self.num_batches * self.dataloader.batch_size)
            / self.total_time,
        }
...
def load_csv_dataset(
    ...
    # Load dataset
    dataset = CSVDataset(
        file_path, target_column=target_column, feature_columns=feature_columns
    )
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=32, shuffle=True, transform=transform
    )
    val_loader = DataLoader(
        val_dataset, batch_size=32, shuffle=False, transform=transform
    )
    test_loader = DataLoader(
        test_dataset, batch_size=32, shuffle=False, transform=transform
    )
