...
"""
        ...
        for node in graph.topological_sort():
            ...
            if node.op_type == "Add":
                ...
                if len(node.inputs[0].shape) == 2 and len(node.inputs[1].shape) == 2:
                    ...
                    if k != k2:
                        raise ValueError(
                            f"Incompatible dimensions for MatMul: {node.inputs[0].shape} and {node.inputs[1].shape}"
                        )

                    c_type = self._convert_type(node.dtype)
                    c_code += f"    {c_type}* {var_name} = ({c_type}*)malloc(sizeof({c_type}) * {m * n});\n"
                    c_code += (
                        f"    memset({var_name}, 0, sizeof({c_type}) * {m * n});\n"
                    )
                    ...
                    raise NotImplementedError(
                        f"Unsupported shapes for MatMul: {node.inputs[0].shape} and {node.inputs[1].shape}"
                    )
                ...
                if not node.shape or len(node.shape) <= 1:
                    ...
                    c_code += f"    for (int i = 0; i < {size}; i++) {{\n"
                    c_code += (
                        f"        {var_name}[i] = {input1}[i] > 0 ? {input1}[i] : 0;\n"
                    )
                    c_code += "    }\n"
                else:
                ...
                if not node.shape or len(node.shape) <= 1:
                    ...
                    c_code += f"    for (int i = 0; i < {size}; i++) {{\n"
                    c_code += (
                        f"        {var_name}[i] = 1.0 / (1.0 + exp(-{input1}[i]));\n"
                    )
                    c_code += "    }\n"
                else:
        ...
        for node_id, var_name in variable_map.items():
            if node_id not in [n.id for n in graph.inputs] and var_name not in [
                f"output{i}" for i in range(len(graph.outputs))
            ]:
    ...
    def compile(self, graph: IRGraph) -> Any:
        ...
        def execute(*args):
            ...
            for i, output_node in enumerate(graph.outputs):
                ...
                output_ptrs.append(
                    output.ctypes.data_as(argtypes[len(graph.inputs) + i])
                )
