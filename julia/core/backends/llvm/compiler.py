...
class LLVMCompiler:
    ...
    def _convert_type(
        self, dtype: DataType, shape: Tuple[int, ...]
    ) -> (
        ir.Type
    ):  # TODO This needs a cleaner solution I just want it started (like most of it atp)
    ...
    def _create_module(self, graph: IRGraph) -> ir.Module:
        ...
        input_types = [
            self._convert_type(node.dtype, node.shape) for node in graph.inputs
        ]
        # Output: pointer to output buffer
        output_types = [
            self._convert_type(node.dtype, node.shape) for node in graph.outputs
        ]
            ...
            if node.op_type == "Add":
                ...
                if not node.shape or len(node.shape) <= 1:
                    ...
                    raise NotImplementedError(
                        "Multi-dimensional array addition not yet implemented"
                    )

            elif (
                node.op_type == "Mul"
            ):  # Might need a lot more op type util than anticipated
                lhs = value_map[node.inputs[0].id]
                rhs = value_map[node.inputs[1].id]
        ...
        for i, output_node in enumerate(graph.outputs):
            ...
            if not isinstance(result.type, ir.PointerType) and isinstance(
                output_ptr.type, ir.PointerType
            ):
                builder.store(result, output_ptr)
            # If both are pointers, load from result first
            elif isinstance(result.type, ir.PointerType) and isinstance(
                output_ptr.type, ir.PointerType
            ):
            ...
            elif isinstance(result.type, ir.PointerType) and not isinstance(
                output_ptr.type, ir.PointerType
            ):
                loaded_result = builder.load(result)
                # Output should be a pointer
                raise TypeError(
                    f"Output is not a pointer but result is: {result.type} vs {output_ptr.type}"
                )
            else:
                # Both are not pointers???
                raise TypeError(
                    f"Both result and output are not pointers: {result.type} vs {output_ptr.type}"
                )
    ...
    def compile(self, graph: IRGraph) -> Any:
        ...
        def execute(*args):
            ...
            if len(args) != len(graph.inputs):
                raise ValueError(
                    f"Expected {len(graph.inputs)} inputs, got {len(args)}"
                )
