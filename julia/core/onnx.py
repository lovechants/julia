...
class ONNXImporter:
    ...
    def import_from_model(self, model: Any) -> IRGraph:
        ...
        for input_info in model.graph.input:
            ...
            node = graph.add_placeholder(
                shape=tuple(shape), dtype=dtype, name=input_info.name
            )
...
class ONNXExporter:
    ...
    def export_to_model(self, graph: IRGraph, opset_version: int = 13) -> Any:
        ...
        nodes_without_shape = [
            node for node in graph.nodes.values() if node.shape is None
        ]
        if nodes_without_shape:
            print(
                f"Warning: The following nodes don't have shapes inferred: {[node.name for node in nodes_without_shape]}"
            )
            graph.infer_shapes()
            ...
            if nodes_without_shape:
                nodes_str = ", ".join(
                    [f"{node.name} ({node.op_type})" for node in nodes_without_shape]
                )
                raise ValueError(
                    f"After shape inference, these nodes still have no shapes: {nodes_str}"
                )

        # Make sure all nodes have types
        nodes_without_type = [
            node for node in graph.nodes.values() if node.dtype is None
        ]
        if nodes_without_type:
            nodes_str = ", ".join(
                [f"{node.name} ({node.op_type})" for node in nodes_without_type]
            )
            raise ValueError(
                f"The following nodes don't have data types inferred: {nodes_str}"
            )
        ...
        for input_node in graph.inputs:
            ...
            for dim in input_node.shape:
                shape.append(
                    onnx.TensorShapeProto.Dimension(dim_value=dim if dim > 0 else 1)
                )

            tensor_type = onnx.TypeProto.Tensor(
                elem_type=elem_type, shape=onnx.TensorShapeProto(dim=shape)
            )
            type_proto = onnx.TypeProto(tensor_type=tensor_type)

            # Create value info
            value_info = onnx.ValueInfoProto(name=input_node.name, type=type_proto)
        ...
        for node_id, node in graph.nodes.items():
            if isinstance(node, ConstantNode):
                ...
                tensor = numpy_helper.from_array(
                    node.attributes["value"], name=node.name
                )
        ...
        for node in all_nodes:
            ...
            if missing_inputs:
                raise ValueError(
                    f"Node {node.name} ({node.op_type}) has inputs that have not been processed: "
                    f"{[input_node.name for input_node in missing_inputs]}"
                )

            # Get input names
            input_names = [
                processed_outputs[input_node.id] for input_node in node.inputs
            ]
            ...
            if custom_converter:
                ...
                if hasattr(onnx_node, "output") and onnx_node.output:
                    ...
                    processed_outputs[node.id] = (
                        node.name or f"{node.op_type}_{node.id[:8]}"
                    )
            else:
                try:
            ...
            if node.shape is not None and node.dtype is not None:
                ...
                value_info = onnx.ValueInfoProto(
                    name=processed_outputs[node.id], type=type_proto
                )
        ...
        for output_node in graph.outputs:
            if output_node.id not in processed_outputs:
                raise ValueError(
                    f"Output node {output_node.name} was not processed during graph traversal"
                )
            ...
            # Create value info
            value_info = onnx.ValueInfoProto(name=output_name, type=type_proto)
