...
class ScaledDotProductAttention(Function):
    ...
    @staticmethod
    def forward(
        ctx, query, key, value, mask=None, scale=None, dropout_p=0.0, training=True
    ):
        """
        ...
        if mask is not None:
            ...
            if mask_data.shape != scores_data.shape:
                ...
                if len(q_shape) == 4 and len(mask_data.shape) == 2:
                    ...
                    if (
                        mask_data.shape[0] == batch_size
                        and mask_data.shape[1] == seq_len_q
                    ):
                        ...
                        mask_data = np.broadcast_to(
                            mask_data[None, :, :],
                            (effective_batch_size, seq_len_q, seq_len_k),
                        )
                    else:
                        raise ValueError(
                            f"Cannot handle mask shape {mask_data.shape} with query shape {q_shape}"
                        )
                    ...
                    if (
                        ...
                        mask_data = np.broadcast_to(
                            mask_data[None, :, :], (batch_size, seq_len_q, seq_len_k)
                        )
                    else:
                        raise ValueError(
                            f"Cannot handle mask shape {mask_data.shape} with query shape {q_shape}"
        ...
        if training and dropout_p > 0:
            dropout_mask = (
                np.random.rand(*attention_weights_data.shape) > dropout_p
            ).astype(np.float32)
            attention_weights_data = (
                attention_weights_data * dropout_mask / (1 - dropout_p)
            )
        ...
        if len(q_shape) == 4:
            ...
            attention_weights_data = attention_weights_data.reshape(
    ...
    def backward(ctx, grad_output, grad_attention_weights=None):
        ...
        if len(q_shape) == 4:
            ...
            grad_out_reshaped = grad_output.data.reshape(
                effective_batch_size, seq_len_q, d_v
            )
            attention_reshaped = attention_weights.reshape(
                effective_batch_size, seq_len_q, seq_len_k
            )
        ...
        for b in range(effective_batch_size):
            ...
            grad_value_data[b] = np.matmul(
                attention_reshaped[b].T, grad_out_reshaped[b]
            )
...
class GroupedKVExpansion(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_data = grad_output.data.reshape(
            batch_size, original_heads, expansion_factor, seq_len, d_k
        )
...
class RotaryPositionalEmbedding(Function):
    ...
    def forward(ctx, x, cos, sin):
        ...
        rotated_data = np.concatenate(
            [x1 * cos.data - x2 * sin.data, x1 * sin.data + x2 * cos.data], axis=-1
        )
    ...
    def backward(ctx, grad_output):
        ...
        grad_x_data = np.concatenate(
            [grad1 * cos.data + grad2 * sin.data, -grad1 * sin.data + grad2 * cos.data],
            axis=-1,
        )
...
def scaled_dot_product_attention_autograd(
    query, key, value, mask=None, dropout_p=0.0, training=True
):
    """
    ...
    return ScaledDotProductAttention.apply(
        query, key, value, mask, None, dropout_p, training
    )


def multi_head_attention_autograd(
    query,
    key,
    value,
    num_heads,
