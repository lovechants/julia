...
class RNN(Layer):
    ...
    def __init__(
        ...
        for layer in range(1, num_layers):
            for direction in range(self.num_directions):
                layer_input_size = (
                    hidden_size * self.num_directions if layer > 0 else input_size
                )
            ...
            for layer in range(self.num_layers):
                ...
                for direction in range(self.num_directions):
                    ...
                    if direction == 1:  # Backward direction
                        x_t = Tensor(
                            x.data[seq_len - 1 - t], requires_grad=x.requires_grad
                        )
                    ...
                    if layer == 0:
                        ...
                        if self.bidirectional and direction == 0:
                            ...
                            if isinstance(
                                prev_outputs, list
                            ):  # Handle bidirectional outputs
                                combined = Tensor(
                                    np.concatenate(
                                        [prev_outputs[0].data, prev_outputs[1].data],
                                        axis=1,
                                    ),
                                    requires_grad=True,
                                )
...
class LSTMCell(Layer):
    ...
    def __init__(self, input_size: int, hidden_size: int, bias: bool = True):
        ...
        # Input gate weights
        self.weight_ii = Tensor(
            np.random.uniform(-k, k, (hidden_size, input_size)), requires_grad=True
        )
        self.weight_hi = Tensor(
            np.random.uniform(-k, k, (hidden_size, hidden_size)), requires_grad=True
        )

        # Forget gate weights
        self.weight_if = Tensor(
            np.random.uniform(-k, k, (hidden_size, input_size)), requires_grad=True
        )
        ...
        # Cell gate weights
        self.weight_ig = Tensor(
            np.random.uniform(-k, k, (hidden_size, input_size)), requires_grad=True
        )
        self.weight_hg = Tensor(
            np.random.uniform(-k, k, (hidden_size, hidden_size)), requires_grad=True
        )

        # Output gate weights
        self.weight_io = Tensor(
            np.random.uniform(-k, k, (hidden_size, input_size)), requires_grad=True
        )
    ...
    def forward(
        ...
        if hx is None:
            h_0 = Tensor(
                np.zeros((batch_size, self.hidden_size)), requires_grad=x.requires_grad
            )
            c_0 = Tensor(
                np.zeros((batch_size, self.hidden_size)), requires_grad=x.requires_grad
            )
        ...
        i_t = x.matmul(self.weight_ii.transpose()) + h_0.matmul(
            self.weight_hi.transpose()
        )
        ...
        f_t = x.matmul(self.weight_if.transpose()) + h_0.matmul(
            self.weight_hf.transpose()
        )
        ...
        # Calculate cell gate
        g_t = x.matmul(self.weight_ig.transpose()) + h_0.matmul(
            self.weight_hg.transpose()
        )
        ...
        o_t = x.matmul(self.weight_io.transpose()) + h_0.matmul(
            self.weight_ho.transpose()
        )
...
class GRUCell(Layer):
    ...
    def __init__(self, input_size: int, hidden_size: int, bias: bool = True):
        ...
        # New gate weights
        self.weight_in = Tensor(
            np.random.uniform(-k, k, (hidden_size, input_size)), requires_grad=True
        )
        self.weight_hn = Tensor(
            np.random.uniform(-k, k, (hidden_size, hidden_size)), requires_grad=True
        )
    ...
    def forward(self, x: Tensor, hidden: Optional[Tensor] = None) -> Tensor:
        ...
        r_t = x.matmul(self.weight_ir.transpose()) + hidden.matmul(
            self.weight_hr.transpose()
        )
        ...
        z_t = x.matmul(self.weight_iz.transpose()) + hidden.matmul(
            self.weight_hz.transpose()
        )
        ...
        n_t = x.matmul(self.weight_in.transpose()) + (r_t * hidden).matmul(
            self.weight_hn.transpose()
        )
