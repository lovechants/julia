...
class Layer:
            ...
            if recurse and hasattr(module, "_modules"):
                ...
                for submodule_name, submodule in submodules.items():
                    if submodule is not None:
                        collect_params(
                            submodule, current_params_set, current_modules_visited
                        )
    ...
    def __setattr__(self, name: str, value: Any):
        if (
            "_parameters" not in self.__dict__
            or "_modules" not in self.__dict__
...
class Linear(Layer):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.weight = Tensor(
            np.random.randn(out_features, in_features) * scale, requires_grad=True
        )
    ...
    def set_weight(self, new_weight_data):
        ...
        if new_weight_data.shape != self.weight.shape:
            raise ValueError(
                f"New weight shape {new_weight_data.shape} doesn't match expected {self.weight.shape}"
            )
    ...
    def set_bias(self, new_bias_data):
        ...
        if new_bias_data.shape != self.bias.shape:
            raise ValueError(
                f"New bias shape {new_bias_data.shape} doesn't match expected {self.bias.shape}"
            )
...
class Sequential(Layer):
    ...
    def __init__(self, *layers):
        ...
        if not hasattr(self, "_modules") or self._modules is None:
            ...
            raise RuntimeError(
                "Sequential parent Layer not initialized correctly (_modules missing)"
            )
        ...
        for i, layer in enumerate(layers):
            ...
            if isinstance(layer, Layer):
                ...
                raise TypeError(
                    f"Sequential layer expected Layer instance or callable, got {type(layer)}"
                )
...
class Dropout(Layer):
    ...
    def __init__(self, p: float = 0.5, inplace: bool = False):
        ...
        if p < 0 or p > 1:
            raise ValueError(
                f"Dropout probability must be between 0 and 1, given probablity {p}"
            )
        self.p = p
...
class Flatten(Layer):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        new_shape = (
            new_shape[: self.start_dim]
            + [flat_dim_size]
            + new_shape[self.end_dim + 1 :]
        )
