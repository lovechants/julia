...
class MultiHeadAttention(Layer):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, 
        ...
        """
        from julia.core.ops_attention import ScaledDotProductAttention, MultiHeadReshape, MultiHeadMerge
        ...
        output, attention_weights = ScaledDotProductAttention.apply(
            Q, K, V, mask, self.scale, self.dropout if self.training else 0.0, self.training
        )
...
class GroupedQueryAttention(Layer):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, 
        ...
        Q = MultiHeadReshape.apply(Q, self.num_query_heads)  # (batch_size, num_query_heads, seq_len, d_k)
        K = MultiHeadReshape.apply(K, self.num_kv_heads)     # (batch_size, num_kv_heads, seq_len, d_k)
        V = MultiHeadReshape.apply(V, self.num_kv_heads)     # (batch_size, num_kv_heads, seq_len, d_k)
        ...
        # Scaled dot-product attention
        output, attention_weights = ScaledDotProductAttention.apply(
            Q, K_expanded, V_expanded, mask, self.scale, self.dropout if self.training else 0.0, self.training
        )
...
class MultiHeadLatentAttention(Layer):
    ...
    def __init__(self, d_model: int, num_heads: int = 8, latent_dim: int = 256, dropout: float = 0.1):
        super().__init__()
...
def create_attention_mask(causal: bool = False, seq_len: int = None, 
                         padding_lengths: list = None) -> Tensor:
    """
    ...
    if masks:
        ...
        for mask in masks[1:]:
            combined_mask = Tensor(
                np.logical_or(combined_mask.data, mask.data), 
                requires_grad=False
            )
