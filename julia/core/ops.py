...
class Transpose(Function):
    ...
    def forward(ctx: Context, tensor: Tensor) -> Tensor:
        ...
        return Tensor(
            result_data
        )  # Let Function.apply handle requires_grad and _backward_node
...
class Log(Function):
    ...
    def forward(ctx, a):
        ...
        result = np.log(
            a.data + 1e-8
        )  # Epsilon for stability #TODO pure math functions for calculations
        return Tensor(result, requires_grad=a.requires_grad)
...
class MatMul(Function):
    ...
    def forward(ctx, a, b):
        ctx.save_for_backwards(a, b)
        return Tensor(
            np.matmul(a.data, b.data), requires_grad=a.requires_grad or b.requires_grad
        )
    ...
    def backward(ctx, grad_output: Tensor) -> tuple[Tensor | None, Tensor | None]:
        ...
        if grad_a_data.shape != a.shape:
            ...
            print(
                f"Warning: MatMul grad_a shape mismatch {grad_a_data.shape} vs {a.shape}"
            )
        ...
        if grad_b_data.shape != b.shape:
            print(
                f"Warning: MatMul grad_b shape mismatch {grad_b_data.shape} vs {b.shape}"
            )
...
class MatMulTranspose(Function):
    ...
    def forward(ctx, a, b):
        ...
        b_t = b.data.T
        return Tensor(
            np.matmul(a.data, b_t), requires_grad=a.requires_grad or b.requires_grad
        )
...
class LogSoftmax(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad = (
            grad_output.data
            - np.sum(grad_output.data * softmax, axis=dim, keepdims=True) * softmax
        )
...
class Swish(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad = grad_output.data * (
            beta * swish + sigmoid_x * (1 - beta * x * sigmoid_x)
        )
        return (Tensor(grad), None)
...
def extend_tensor_with_missing_ops():
    ...
    Tensor.var = lambda self, dim=None, keepdim=False, unbiased=True: Var.apply(
        self, dim, keepdim, unbiased
    )
