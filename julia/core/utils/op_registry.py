...
class OpRegistry:
    ...
    def infer_shape(
        self, node, input_shapes: List[Tuple[int, ...]]
    ) -> Optional[Tuple[int, ...]]:
        """
...
def infer_reshape_shape(node, input_shapes):
    ...
    if len(input_shapes) > 1:
        ...
        if (
            hasattr(node.inputs[1], "attributes")
            and "value" in node.inputs[1].attributes
        ):
            new_shape = tuple(
                int(x) for x in node.inputs[1].attributes["value"].flatten()
            )
