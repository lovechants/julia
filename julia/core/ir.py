...
class IRNode:
    ...
    """
    def __init__(self, op_type: str, inputs: List["IRNode"] = None, 
                 attributes: Dict[str, Any] = None, name: str = None):
    ...
    def transpose(self):
        ...
        if hasattr(self, 'shape') and self.shape:
            ...
            if len(self.shape) < 2:
                raise ValueError(f"Cannot transpose node with shape {self.shape} - needs at least 2 dimensions")
...
class VariableNode(IRNode):
    ...
    def __init__(self, shape: Tuple[int, ...], dtype: DataType = DataType.FLOAT32, 
                 name: str = None, trainable: bool = True):
        super().__init__(
            op_type="Variable",
            inputs=[],
...
class IRGraph:
    ...
    def add_placeholder(self, shape: Tuple[int, ...], dtype: DataType = DataType.FLOAT32, 
                        name: str = None) -> IRNode:
    ...
    def add_variable(self, shape: Tuple[int, ...], dtype: DataType = DataType.FLOAT32, 
                     name: str = None, trainable: bool = True) -> IRNode:
        """Add a variable node"""
    ...
    def add_constant(self, value: Union[np.ndarray, Tensor], name: str = None) -> IRNode:
        """Add a constant node"""
    ...
    def infer_shapes(self):
        ...
        print(f"Topological order for shape inference: {[node.name for node in topo_order]}")
        ...
        nodes_without_shape = [node for node in self.nodes.values() if node.shape is None]
