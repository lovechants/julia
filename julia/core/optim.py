...
class CosineAnnealingLR(LRScheduler):
    ...
    def __init__(
        self, optimizer, T_max: int, eta_min: float = 0, last_epoch=-1, verbose=False
    ):
        self.T_max = T_max
...
class OptimizerBase:
    ...
    def load_checkpoint(self, filepath):
        ...
        if checkpoint["optimizer_class"] != self.__class__.__name__:
            raise ValueError(
                f"Checkpoint is for {checkpoint['optimizer_class']}, "
...
class SGD:
    def __init__(
        self, params, lr=0.01, momentum=0.0, dampening=0, weight_decay=0, nesterov=False
    ):
        """
...
class Muon:
    ...
    def step(self):
        for i, param in enumerate(self.params):
            ...
            self.velocity[i] = (
                self.momentum * self.velocity[i] - self.lr * param.grad.data
            )
...
class Adam:
    ...
    def step(self):
        ...
        for i, param in enumerate(self.params):
            ...
            exp_avg, exp_avg_sq = (
                self.state[param_id]["exp_avg"],
                self.state[param_id]["exp_avg_sq"],
            )
...
class AdamW:
    ...
    def step(self):
        ...
        for i, param in enumerate(self.params):
            ...
            # Update parameters
            param.data = (1 - self.weight_decay * self.lr) * param.data - step_size * (
