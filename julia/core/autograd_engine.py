...
class AutogradEngine:
    ...
    def backward(
        self, root_tensor, gradient=None, retain_graph=False, create_graph=False
    ):
    ...
    def _execute_node_backward(self, tensor, gradients, create_graph):
        ...
        try:
            ...
            for input_tensor, input_grad in zip(node.inputs, input_grads):
                if (
                    isinstance(input_tensor, TensorClass)
                    and input_tensor.requires_grad
                    and input_grad is not None
                ):
                    input_id = id(input_tensor)
                    grad_data = (
                        input_grad.data if hasattr(input_grad, "data") else input_grad
                    )
    ...
    def _assign_leaf_gradients(self, topo_order, gradients, create_graph):
        ...
        for tensor in topo_order:
            ...
            should_retain = getattr(tensor, "_is_leaf", True) or getattr(
