...
class TransformerBlock(Layer):
    ...
    def __init__(self, d_model, num_heads, d_ff, dropout=0.1):
        ...
        self.feed_forward = Sequential(Linear(d_model, d_ff), Linear(d_ff, d_model))
            ...
            for b in range(batch_size):
                for t in range(seq_len):
                    ...
                    target_tensor = Tensor(
                        np.eye(vocab_size)[target], requires_grad=False
                    )

                    loss = cross_entropy_loss(pred_tensor, target_tensor)
                    batch_loss += loss.data

            batch_loss_tensor = Tensor(
                np.array(batch_loss / (batch_size * seq_len)), requires_grad=True
            )
