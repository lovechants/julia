...
def train_time_series_model():
    ...
    for epoch in range(num_epochs):
        ...
        for batch in range(num_batches):
            ...
            inputs, targets = generate_sine_wave_data(
...
def train_text_classifier():
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            print(
                f"Epoch {epoch+1}/{num_epochs}, Loss: {avg_loss:.4f}, Accuracy: {accuracy:.3f}"
            )
...
def compare_rnn_types():
    ...
    for rnn_type in rnn_types:
        ...
        model = TimeSeriesRNN(
            input_features=1, hidden_size=hidden_size, rnn_type=rnn_type
        )
