...
class BSplineFunction(Function):
    ...
    def forward(ctx, x, control_points, knots, degree=3):
        ...
        return Tensor(
            output, requires_grad=x.requires_grad or control_points.requires_grad
        )
    ...
    def _basis_function(x, i, degree, knots):
        ...
        if denom1 != 0:
            term1 = (
                (x - knots[i])
                / denom1
                * BSplineFunction._basis_function(x, i, degree - 1, knots)
            )
        ...
        if denom2 != 0:
            term2 = (
                (knots[i + degree + 1] - x)
                / denom2
                * BSplineFunction._basis_function(x, i + 1, degree - 1, knots)
            )
        ...
        for i in range(input_dim):
            for j in range(output_dim):
                ...
                self.control_points[cp_key] = Tensor(
                    np.random.randn(n_control_points) * 0.1, requires_grad=True
                )
...
class SimpleKAN(Layer):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, degree=3):
        ...
        self.hidden_coeffs = Tensor(
            np.random.randn(input_dim, hidden_dim, degree + 1) * 0.1, requires_grad=True
        )
...
def train_kan_regression():
    ...
    model = SimpleKAN(input_dim=2, hidden_dim=10, output_dim=1, degree=3)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 20 == 0:
            ...
            print(
                f"Epoch {epoch+1}/{num_epochs}, Train Loss: {avg_loss:.6f}, Test Loss: {test_loss.data:.6f}"
            )
...
def train_kan_classification():
    ...
    # Create model
    model = SimpleKAN(
        input_dim=2, hidden_dim=8, output_dim=2, degree=2  # Binary classification
    )
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 30 == 0:
            ...
            print(
                f"Epoch {epoch+1}/{num_epochs}, Train Acc: {train_acc:.3f}, Test Acc: {test_acc:.3f}"
            )
